...
function UpdateDisplayNameModal(props: UpdateDisplayNameModalProps) {
  ...
  const updateUserProfileMutation = useMutation({
    mutationFn: (request: UpdateDisplayNamePayloadType) => updateUserDisplayName(request),
    onSuccess: () => {
      toast({
        variant: 'success',
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle className="mb-2">{`${authUser?.displayName ? 'Update' : 'Add'} display name`}</DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form className="space-y-6" onSubmit={form.handleSubmit(onPressSubmit)}>
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Input {...field} placeholder="Display name*" disabled={updateUserProfileMutation.isPending} />
                  <FormMessage />
                </FormItem>
              )}
            />
            <DialogFooter>
              ...
              <Button
                ...
                type="submit"
                disabled={updateUserProfileMutation.isPending || !form.formState.isDirty}
              >
                {updateUserProfileMutation.isPending && <Spinner className="h-5 w-5 text-white" />}
